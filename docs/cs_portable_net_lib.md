...
```csharp
IBibcodeQueryBindingController bibcodeQueryBinding = client.BibcodeQueryBinding;
```
...
```csharp
Task<Models.ReturnBibcodeModel> GetBibcode(string bibcode, string dbKey, string dataType)
```
...
Models.ReturnBibcodeModel result = await bibcodeQueryBinding.GetBibcode(bibcode, dbKey, dataType);
